        ...
        if (enemy.radius - 10 > 5 && player.powerUp != 'One Shot') {
          ...
          setTimeout(() => {
            ...
            if (enemyFound) {
              ...
              projectiles.splice(projectileIndex, 1)
            } else if (player.powerUp === 'One Shot' || projectile.color === 'red') {
              enemies.splice(index, 1)
              projectiles.splice(projectileIndex, 1)
            }
          }, 0)
...
document.getElementById("lbBtn").addEventListener('click', () => {
  loadJSON("https://zball-ec41.restdb.io/rest/leaderboard", "6228c7c7dced170e8c83a0b8", getLeaderboard, setError);
  let leaderboardModalEl = document.getElementById("leaderboardModalEl")
  gsap.to('#whiteModalEl', {
    opacity: 0,
    scale: 0.75,
    duration: 0.25,
    ...
    onComplete: () => {
      modalEl.style.display = 'none'
      leaderboardModalEl.style.display = 'flex'
      gsap.to('#leaderboardWhiteModalEl', {
        opacity: 1,
        scale: 1,
        duration: 0.45,
        ease: 'expo'
